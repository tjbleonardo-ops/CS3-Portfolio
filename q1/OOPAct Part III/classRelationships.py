...
#individual task print    
  def __str__(self):
     return f"{self.task_name}, {self.task_type}, due in {self.__deadline} day/s. {'Completed.' if self.__is_completed else 'Not Completed'}, Subject is {self.task_subject}."
     ...
     while True:
      ...
      if edit_choice == "Yes":
            ...
            if choice == "1":
               ...
               self.task_subject = new_task_subject
            elif choice == "5":
   ...
   def add_task(tasks: list, subjects: list):
      for task in tasks:
         ...
         for subject in subjects:
            if task.task_subject.upper() == subject.subject_name.upper():
...
task4 = Task("OOPAct Part IV", "FA", 4, False, "ComSci")
